...
const ChaptersForm: NextPage<Props> = ({ initialData, courseId }) => {
  ...
  const onReorder = async (updateData: { id: string; position: number }[]) => {
    try {
      setIsUpdating(true)
      await axios.put(`/api/cursos/${courseId}/capitulos/reordenar`, {
        list: updateData,
      })
  ...
  const onEdit = (id: string) => {
    router.push(`/app/professor/cursos/${courseId}/capitulos/${id}`)
  }

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/cursos/${courseId}/capitulos`, values)
  ...
  return (
    <div className='relative mt-6 border  rounded-md p-4'>
      ...
      {!isCreating && <p className='text-sm mt-4 text-muted-foreground'>Clica e arrasta para reordenar os capítulos</p>}
    </div>
  )
}
